...
public final class KubectlClient: ContextListingService, NamespaceListingService, PodListingService, DeploymentListingService, ServiceListingService, EventListingService, GenericResourceListingService, PodLogService, UnifiedServiceLogService, UnifiedDeploymentLogService, ManifestService, ManifestValidationService, ResourceWriteService, @unchecked Sendable {
    ...
    public func listPods(
        ...
        if let agent = resolvedK8sAgentPath() {
            do {
                pods = try await RuneK8sAgentWorkloadClient.listPods(
                    executablePath: agent,
                    runner: runner,
                    environment: env,
                    contextName: context.name,
        ...
        var merged = pods
        var didMergeMetrics = false
        if let agent = resolvedK8sAgentPath(),
           let metrics = try? await RuneK8sAgentOperationsClient.podTopByName(
                executablePath: agent,
                runner: runner,
                environment: env,
                contextName: context.name,
            ...
            didMergeMetrics = !metrics.isEmpty
        }
        if !didMergeMetrics,
           let top = await runKubectlAllowFailure(
    ...
    public func listPodsAllNamespaces(
        ...
        if let agent = resolvedK8sAgentPath() {
            do {
                pods = try await RuneK8sAgentWorkloadClient.listPodsAllNamespaces(
                    executablePath: agent,
                    runner: runner,
                    environment: env,
                    contextName: context.name,
        ...
        var didMergeMetrics = false
        if let agent = resolvedK8sAgentPath(),
           let metrics = try? await RuneK8sAgentOperationsClient.podTopByNamespaceAndName(
                executablePath: agent,
                runner: runner,
                environment: env,
                contextName: context.name,
    ...
    public func resourceYAML(
        ...
        if let agent = resolvedK8sAgentPath() {
            do {
                return try await RuneK8sAgentOperationsClient.resourceYAML(
                    executablePath: agent,
                    runner: runner,
                    environment: env,
                    contextName: context.name,
    ...
    public func resourceDescribe(
        ...
        if let agent = resolvedK8sAgentPath() {
            do {
                return try await RuneK8sAgentOperationsClient.resourceDescribe(
                    executablePath: agent,
                    runner: runner,
                    environment: env,
                    contextName: context.name,
                ...
                try rethrowCancellationIfNeeded(error)
                // Continue with kubectl fallback for development builds without a working agent.
            }
        }
        let result = try await runKubectl(
    ...
    public func startPodTerminalSession(
        ...
        if let agent = resolvedK8sAgentPath() {
            executable = agent
            arguments = RuneK8sAgentOperationsClient.podInteractiveShellArguments(
                contextName: context.name,
                namespace: namespace,
                podName: podName,
                container: container,
    ...
    public func validateResourceYAML(
        ...
        if let agent = resolvedK8sAgentPath() {
            do {
                try await RuneK8sAgentOperationsClient.validateFile(
                    executablePath: agent,
                    runner: runner,
                    environment: env,
                    contextName: context.name,
                ...
                // Continue with kubectl validation only if the agent failure did not contain a usable Kubernetes error.
            }
        }

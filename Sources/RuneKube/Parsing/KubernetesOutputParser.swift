...
public struct KubernetesOutputParser {
    public init() {}
    ...
    /// Fallback parser for legacy pod table output.
    ...
    /// Fallback parser for legacy all-namespace pod table output.
    ...
    /// Single Kubernetes Pod JSON document (same shape as a list item).
    ...
    /// metrics.k8s.io all-namespace pod output: `NAMESPACE  NAME  CPU  MEM`.
    ...
    /// metrics.k8s.io node output: `NAME CPU(cores) CPU% MEMORY(bytes) MEMORY%`.
    ...
    /// Legacy deployment table parser.
    ...
    /// Legacy all-namespace deployment table parser.
    ...
    /// Parses legacy jobs table output (tab-separated columns).
    ...
    /// Parses legacy cronjobs table output (tab-separated; schedule may contain spaces).
    ...
    public func parseJobs(namespace: String, from raw: String) throws -> [ClusterResourceSummary] {
        let items = try KubernetesListJSON.decodeLenientListItems(
            KubeJobItem.self,
            from: raw,
